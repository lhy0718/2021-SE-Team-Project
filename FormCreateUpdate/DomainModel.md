...
User가 학생 추가버튼을 클릭한다. | D | 학생추가폼 요청
학생 정보|	K	|수업 정보 
User가 학생 추가 완료 버튼을 클릭한다.|	D|	수업추가 요청
서버와 연결해 User가 입력한 수업정보를 DB에 전송하고 record값을 받아온다.|D|	DB Connection
UI를 통해 현재 상태를 화면에 표현한다.|	D|	Page Marker
현재 상태를 보여주는 UI|	K	|View
...
Controller <-> Page Marker|	Controller는 Page Marker에게 요청을 보내고 UI를 받는다.|	요청 전달
Controller <-> View|	Controller는 View에게 게시할 내용을 전달한다.|	게시
학생 정보 <-> Controller| 학생 정보를 Controller에게 제공한다.|	데이터 전달
Page Marker <-> View|	Page Marker가 View를 준비한다.	|준비
Controller <-> DB Connection|	Controller는 DB Connection으로 request를 넘긴다.|	요청 전달
학생 추가 요청 <-> Controller|학생 추가 요청을 Controller에게 전달한다.|	요청 전달
Page Marker <-> DB Connection|	DB Connection에서 받아온 값을 넘기고, Page Marker는 이 데이터를 화면에 표현한다.	|데이터 전달

### Extracting Attributes
Concept|Attributes|Attributes Description
-|-|-
학생정보|학생 정보 record|사용자가 입력한 학생 정보(학생명, 학번, 학부모 email주소)

![diagram (1)_5](https://user-images.githubusercontent.com/64057843/115826307-a271d580-a445-11eb-84d0-ce68e1058a6d.png)
...
Responsibility Description|Type|Concept Name
-|-|-
UC-5와 연관된 개념들과 행동들을 조정하고 다른 개념에 작업을 위임 | D | Controller
User가 학생 수정버튼을 클릭한다. | D | 학생 수정폼 요청
학생 정보|	K	|학생 정보 
User가 학생 수정 완료 버튼을 클릭한다.|	D|	수업수정 요청
서버와 연결해 User가 입력한 학생정보를 DB에 전송하고 record값을 받아온다.|D|	DB Connection
UI를 통해 현재 상태를 화면에 표현한다.|	D|	Page Marker
현재 상태를 보여주는 UI|	K	|View
...
Controller <-> View|	Controller는 View에게 게시할 내용을 전달한다.|	게시
학생 정보 <-> Controller|	 정보를 Controller에게 제공한다.|	데이터 전달
Page Marker <-> View|	Page Marker가 View를 준비한다.	|준비
Controller <-> DB Connection|	Controller는 DB Connection으로 request를 넘긴다.|	요청 전달
학생 수정 요청 <-> Controller|학생 수정 요청을 Controller에게 전달한다.|	요청 전달
Page Marker <-> DB Connection|	DB Connection에서 받아온 값을 넘기고, Page Marker는 이 데이터를 화면에 표현한다.	|데이터 전달
...
![diagram (1)_6](https://user-images.githubusercontent.com/64057843/115826646-32b01a80-a446-11eb-87bf-06b79b2d1bfb.png)
...
Responsibility Description|Type|Concept Name
-|-|-
UC-6과 연관된 개념들과 행동들을 조정하고 다른 개념에 작업을 위임 | D | Controller
User가 학생 삭제버튼을 클릭한다. | D | 학생 삭제 요청
서버와 연결해 User가 입력한 수업정보를 DB에 전송하고 record값을 받아온다.|D|	DB Connection
현재 상태를 보여주는 UI|	K	|View
UI를 통해 현재 상태를 화면에 표현한다.|	D|	Page Marker
삭제 성공 여부를 확인한다.|	D|	Valid Checker
...
Page Marker <-> View|	Page Marker가 View를 준비한다.|	준비
Controller<->DB Connection|	Controller는 DB Connection으로 request를 넘긴다.|요청전달
학생 삭제 요청 <-> Controller|학생 삭제 요청을 Controller에게 전달한다.|	요청 전달
Page Marker <-> DB Connection|	DB Connection에서 받아온 값을 넘기고, Page Marker는 이 데이터를 화면에 표현한다.|	데이터 전달
Valid Checker<-> DB connection|	DB connection에서 탈퇴 요청된 데이터가 잘 제거되어있는지의 여부를 Valid Checker애 넘겨준다.|	유효 여부 전달
Valid Checker<-> Pop-up Maker|	Pop-up Maker는 Valid Checker에게 정보를 받아 성공/실패에 관한 팝업창을 만든다.|생성
...
![diagram (1)_7](https://user-images.githubusercontent.com/64057843/115826648-33e14780-a446-11eb-9c08-25c81ddfb4ec.png)
